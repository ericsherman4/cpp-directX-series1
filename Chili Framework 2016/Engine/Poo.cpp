...
void Poo::Update()
{
    ...
    // you can do this even though poo doesnt have access to the gfx object from game!
    // because screenwidth and screenheight are static constexpr members of the class 
...
void Poo::ProcessConsumption(const Dude &dude)
{
	...
	if (duderight >= x && dudebottom >= y && dude.GetXPos() <= pooright && dude.GetYPos() <= poobottom )
	{
		isEaten = true;
	}
}
